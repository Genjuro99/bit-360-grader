...
The GitHub Actions workflow builds a debug-signed APK named `BIT360-Restaurant-Grader-v1.0.7-fresh-history-qa-test.apk`. Its debug application ID is `com.believeintaste.bit360grader.historyqatest`, allowing a cache-free first-launch check that a successful Retry clears the failed WebView history entry. Other preserved test builds and their data are not replaced. This is installable for private testing, but it is not a Play Store production release.
