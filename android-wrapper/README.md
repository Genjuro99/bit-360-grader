...
The GitHub Actions workflow builds a debug-signed APK named `BIT360-Restaurant-Grader-v1.0.2-test.apk`. Its debug application ID is `com.believeintaste.bit360grader.test`. This is installable for private testing, but it is not a Play Store production release. Use one stable private test-signing key before treating a future build as an update to an installed test app.
