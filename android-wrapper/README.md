...
The GitHub Actions workflow builds a debug-signed APK named `BIT360-Restaurant-Grader-v1.0.8-back-exit-qa-test.apk`. Its debug application ID is `com.believeintaste.bit360grader.backqatest`, allowing a cache-free first-launch check that Android Back exits the single-page wrapper instead of reopening a failed WebView history entry after Retry. Other preserved test builds and their data are not replaced. This is installable for private testing, but it is not a Play Store production release.
